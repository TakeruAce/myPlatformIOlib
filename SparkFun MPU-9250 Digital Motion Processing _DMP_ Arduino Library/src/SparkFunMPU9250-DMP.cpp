...
inv_error_t MPU9250_DMP::begin(unsigned char mpu_address)
{
	inv_error_t result;
	...
	result = mpu_init(&int_param, mpu_address);
	

	
	Serial.println(mpu_set_bypass(1)); // Place all slaves (including compass) on primary bus
...
inv_error_t MPU9250_DMP::dmpUpdateFifo(void)
{
	...
	int retval = dmp_read_fifo(gyro, accel, quat, &timestamp, &sensors, &more);
